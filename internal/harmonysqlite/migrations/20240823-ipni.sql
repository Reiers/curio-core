...
-- Source: github.com/filecoin-project/curio harmony/harmonydb/sql/20240823-ipni.sql
...
-- Flagged constructs: CREATE FUNCTION (plpgsql)
--
...
CREATE TABLE IF NOT EXISTS ipni (
    order_number INTEGER PRIMARY KEY AUTOINCREMENT, -- Unique increasing order number
    ad_cid TEXT NOT NULL,
    context_id BLOB NOT NULL, -- abi.PieceInfo in Curio
    -- metadata BLOB NOT NULL DEFAULT '\xa01200' (Added in 20250505-market-mk20.sql)
    is_rm INTEGER NOT NULL,

    -- skip added in 20241106-market-fixes.sql
    -- is_skip INTEGER NOT NULL DEFAULT FALSE, -- set to true means return 404 for related entries
...
CREATE TABLE IF NOT EXISTS ipni_chunks (
    ...
    from_car INTEGER NOT NULL, -- Whether the chunk is from a .car file or from the database
    CHECK (
...
--         VALUES (_sp_id, _sector, _reg_seal_proof, _sector_offset, _provider, _context_id, _is_rm, CURRENT_TIMESTAMP, _task_id, FALSE);
--         RETURN;
--     END IF;
-- 
