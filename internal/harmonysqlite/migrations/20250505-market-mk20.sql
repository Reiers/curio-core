...
-- Source: github.com/filecoin-project/curio harmony/harmonydb/sql/20250505-market-mk20.sql
...
-- Flagged constructs: DO $$ block, CREATE FUNCTION (plpgsql), CREATE TRIGGER, COMMENT ON, DROP CONSTRAINT (limited in SQLite), JSONB (mapped to TEXT), ARRAY types (mapped to TEXT/JSON)
--
...
-- ALTER TABLE market_piece_metadata DROP CONSTRAINT IF EXISTS  market_piece_metadata_pkey;

-- Drop the redundant UNIQUE constraint if it exists for market_piece_metadata
-- TODO: PG-DROP-CONSTRAINT. SQLite can't drop a named constraint;
-- recreate the table without the constraint, or accept the old constraint.
-- ALTER TABLE market_piece_metadata DROP CONSTRAINT IF EXISTS market_piece_meta_identity_key;
...
-- TODO: PG-DROP-CONSTRAINT. SQLite can't drop a named constraint;
-- recreate the table without the constraint, or accept the old constraint.
-- ALTER TABLE market_piece_deal DROP CONSTRAINT IF EXISTS market_piece_deal_pkey;

-- Drop the old UNIQUE constraint for market_piece_deal
-- TODO: PG-DROP-CONSTRAINT. SQLite can't drop a named constraint;
-- recreate the table without the constraint, or accept the old constraint.
-- ALTER TABLE market_piece_deal DROP CONSTRAINT IF EXISTS market_piece_deal_identity_key;
...
--     ) THEN
--         ALTER TABLE parked_pieces ADD COLUMN IF NOT EXISTS skip INTEGER NOT NULL DEFAULT FALSE;
--     END IF;
-- END $$;
...
--     ) THEN
--         ALTER TABLE ipni ADD COLUMN IF NOT EXISTS metadata BLOB NOT NULL DEFAULT '\xa01200';
--     END IF;
-- END $$;
...
--     ) THEN
--         ALTER TABLE ipni_chunks ADD COLUMN IF NOT EXISTS is_pdp INTEGER NOT NULL DEFAULT FALSE;
--     END IF;
-- END $$;
...
-- TODO: PG-DROP-CONSTRAINT. SQLite can't drop a named constraint;
-- recreate the table without the constraint, or accept the old constraint.
-- ALTER TABLE ipni_chunks     DROP CONSTRAINT IF EXISTS ipni_chunks_piece_cid_chunk_num_key;
...
--     ) THEN
--         -- TODO: PG-ADD-CONSTRAINT. SQLite can't ADD a CHECK/PRIMARY KEY/FOREIGN KEY
-- constraint to an existing table; fold into the original CREATE TABLE.
-- ALTER TABLE ipni_chunks ADD CONSTRAINT ipni_chunks_piece_cid_is_pdp_chunk_num_key --             UNIQUE (piece_cid, is_pdp, chunk_num);
--     END IF;
-- END $$;
...
-- ) RETURNS VOID AS $$
-- DECLARE
-- _existing_is_rm INTEGER;
--     _latest_is_rm INTEGER;
-- BEGIN
...
--         EXECUTE 'UPDATE market_mk12_deals d SET raw_size = mpd.raw_size FROM market_piece_deal mpd WHERE d.TEXT = mpd.id';
--     END IF;
-- 
...
--         EXECUTE 'UPDATE market_direct_deals d SET raw_size = mpd.raw_size FROM market_piece_deal mpd WHERE d.TEXT = mpd.id';
--     END IF;
-- 
--     IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'market_mk12_deals' AND table_schema = current_schema() AND column_name = 'raw_size') THEN
--         EXECUTE 'UPDATE market_mk12_deals d SET raw_size = p.raw_size FROM market_mk12_deal_pipeline p WHERE d.TEXT = p.TEXT AND d.raw_size IS NULL AND p.raw_size IS NOT NULL';
--     END IF;
-- 
--     IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'market_direct_deals' AND table_schema = current_schema() AND column_name = 'raw_size') THEN
--         EXECUTE 'UPDATE market_direct_deals d SET raw_size = p.raw_size FROM market_mk12_deal_pipeline p WHERE d.TEXT = p.TEXT AND d.raw_size IS NULL AND p.raw_size IS NOT NULL';
--     END IF;
-- END $$;
...
-- (PG COMMENT ON stripped) COMMENT ON COLUMN market_mk20_deal.id IS 'This is ULID TEXT';
-- (PG COMMENT ON stripped) COMMENT ON COLUMN market_mk20_deal.client IS 'Client must always be text as this can be a non Filecoin address like ed25519';
...
CREATE TABLE IF NOT EXISTS market_mk20_pipeline (
    ...
    aggregated INTEGER DEFAULT FALSE,
    ...
    indexing_created_at DATETIME DEFAULT NULL,
...
-- (PG COMMENT ON stripped) COMMENT ON COLUMN market_mk20_pipeline.piece_aggregation IS 'This is set when user sends a aggregated piece. It is also set as `deal_aggregation` when deal is aggregated on SP side.';
-- (PG COMMENT ON stripped) COMMENT ON COLUMN market_mk20_pipeline.deal_aggregation IS 'This is set when user sends a deal with aggregated source. This value is passed to piece_aggregation when aggregation is finished and a single piece remains';
...
-- A separate table helps easier status check, chunked+INTEGER upload support
...
CREATE TABLE IF NOT EXISTS market_mk20_deal_chunk (
    ...
    finalize INTEGER DEFAULT FALSE,
...
-- RETURNS TRIGGER AS $$
-- BEGIN
--     -- Transition into "INTEGER ready" state: chunked=false AND ref_id IS NOT NULL
...
CREATE TABLE IF NOT EXISTS pdp_pipeline (
    ...
    after_add_piece INTEGER DEFAULT FALSE,

    add_message_hash TEXT,
    add_message_index BIGINT NOT NULL DEFAULT 0, -- index of root in the add message

    after_add_piece_msg INTEGER DEFAULT FALSE,

    save_cache_task_id BIGINT DEFAULT NULL,
    after_save_cache INTEGER DEFAULT FALSE,

    indexing INTEGER DEFAULT FALSE,
    indexing_created_at DATETIME DEFAULT NULL,
    indexing_task_id BIGINT DEFAULT NULL,
    indexed INTEGER DEFAULT FALSE,

    announce INTEGER DEFAULT FALSE,
    announce_payload INTEGER DEFAULT FALSE,

    announced INTEGER DEFAULT FALSE,
    announced_payload INTEGER DEFAULT FALSE,
...
CREATE TABLE IF NOT EXISTS piece_cleanup (
    ...
    after_cleanup INTEGER NOT NULL DEFAULT FALSE,
